			...
			<view class="goods-item" :class="{ 'margin-top': index > 0 }" v-for="(item, index) in goodsDatas"
				...
				<view class="flex1 padding-left-sm">
					...
					<view class="pos-r dflex-b padding-top">
						<view class="price flex1">{{ (item.goods.market_price / 100).toFixed(2) || '' }}</view>
		...
		methods: {
			...
			calcTotalMoney() {
				...
				this.goodsDatas.forEach(x => {
					...
					this.goods_money += (x.goods.market_price / 100) * x.goods.goods_num;
				});
				...
				this.$func.usemall.call('order/create', obj).then(res => {
					...
					if (res.code === 200) {
						...
						_this.$api.topay({
							...
							money: (res.datas.money / (1 - 0.2)).toFixed(2),
							type: 'redirect'
						});
						return;
					}
