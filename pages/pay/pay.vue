	...
	<view class="app w-full">
		...
		<view class="pay-type-list">
			...
			<view class="type-item dflex-b pos-r padding-tb-sm" @click="changePayType('头条支付', '微信支付', 'MWEB')">
				...
				<label class="radio">
					<radio value="wxpay" color="#ff6a6c" colors="#ff6a6c"
						:checked="pay_way == '头条支付' && pay_trade_type == 'MWEB'" :disabled="money <= 0" />
					</radio>
				</label>
			</view>
			<!-- #endif -->
			...
			<view class="type-item dflex-b pos-r padding-tb-sm" @click="changePayType('支付宝')">
				...
				<label class="radio">
					<radio value="" color="#FF6A6C" :checked="pay_way == '支付宝'" :disabled="money <= 0" @click="handlePay"/>
					</radio>
				</label>
			</view>
			<!-- <view > -->
				<div v-show="isWxewm" style="margin-left: 40px;">
					<image v-show="isWxewm" src="../../static/images/user/zfb.png" style="width: 250px!important;"></image>
				</div>
			<!-- </view> -->
			...
			<view v-if="time_remaining" class="dflex-c">
				<use-count-down :show-days="false" separator="zh" separator-color="#333" color="#fff" bg-color="#333"
					font-size="24" :timestamp="time_remaining" @end="ontimeend"></use-count-down>
			</view>
			<!-- #endif -->
		</view>
		...
		<view class="padding w-full margin-top-big pos-a" style="bottom: 30rpx;">
			<view class="dflex-b border-radius-big">
				...
				<view class="tac padding-tb-sm flex1 bg-base" 
					@click="confirm">{{pay_tip}}</view>
				<!-- #endif -->

				<!-- #ifdef H5 || MP-360 -->
				<view class="tac padding-tb-sm flex1 bg-base" 
					@click="check">{{pay_tip}}</view>
				<!-- #endif -->
			</view>
		</view>
	</view>
...
<script>
	export default {
		...
		onLoad(options) {
			...
			console.log(options)
			this.money = (options.money / 100).toFixed(2) || 0;
		...
		methods: {
			...
			loadData() {
				...
				_this.$func.usemall.call('order/detail', {
					...
					console.log(res)
					if (res.code == 200) {
						_this.order_state = res.datas.order.order_pay_state;
						_this.orderDetail = res.datas.order;
				...
				uni.requestPayment({
					...
					success: function(pres) {
						...
						if (pres.resultCode !== "9000") {
							_this.$api.msg(pres.memo || '支付未完成，请重新支付');
			...
			check() {
				...
				// let order = {};
				this.orderDetail.order_pay_state = '已完成';
				this.orderDetail.state = '已完成';
				// console.log(this.orderDetail)
				let obj = {};
				obj = this.orderDetail;
				console.log(obj)
				// this.$func.usemall.call('order/orderUpdate', obj).then(res => {
				// 	console.log(res)
				// 	this.$api.msg(res.msg);
				// })
				// // 执行的操作
				// const orderRes = this.db.collection('usemall-order-detail').update(this.orderDetail);
				// console.log('orderRes', orderRes);
				
				uni.redirectTo({
					url: `/pages/pay/success?order_id=${this.order_id}`
				});
